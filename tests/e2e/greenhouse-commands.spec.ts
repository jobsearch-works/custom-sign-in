...
// Skip test execution in CI/automated environments
test.describe.skip("Greenhouse Job Application with Firestore Commands", () => {
  ...
  test("should handle missing commands gracefully", async ({ page }) => {
    // Navigate to the page
    await page.goto("https://boards.greenhouse.io/aurosglobal/jobs/4493809005");

    // Take a screenshot to debug what's on the page
    await page.screenshot({ path: "greenhouse-missing-commands-page.png" });

    // Try to get to the form
    try {
      const applyButton = page.getByRole("link", { name: "Apply Now" });
      if (await applyButton.isVisible({ timeout: 5000 })) {
        console.log("Found Apply Now button, clicking it");
      ...
      console.log("Apply button not found or not needed, proceeding with test");
    }

    // Mock the getDomainConfig to return null
    const firestoreService = new CustomFirestoreService();
    const getDomainConfigOriginal = firestoreService.getDomainConfig;
    firestoreService.getDomainConfig = async (domain: string) => null;

    // Try to fetch domain configuration from Firestore
    const domainConfig = await firestoreService.getDomainConfig(
      "greenhouse.io"
    );
    expect(domainConfig).toBeNull();

    // Test should still complete successfully even without commands
  ...
  test("should extract domain from URL and fetch appropriate commands", async ({
    page,
  }) => {
    // Navigate to the Greenhouse job application page
    await page.goto("https://boards.greenhouse.io/aurosglobal/jobs/4493809005");
    ...
    // Take a screenshot to debug what's on the page
    await page.screenshot({ path: "greenhouse-extract-domain-page.png" });

    // Extract domain from URL
    const url = page.url();
    const domain = new URL(url).hostname.replace(/^boards\./, "");
    ...
    // Mock the getDomainConfig to return test data
    const firestoreService = new CustomFirestoreService();
    const getDomainConfigOriginal = firestoreService.getDomainConfig;
    firestoreService.getDomainConfig = async (domain: string) => {
      return {
    ...
    // Fetch domain configuration using extracted domain
    const domainConfig = await firestoreService.getDomainConfig(domain);
    expect(domainConfig).not.toBeNull();

    // Find any command list for the domain
    expect(domainConfig?.commands.length).toBeGreaterThan(0);
